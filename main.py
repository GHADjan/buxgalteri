...
def export_to_csv(chat_id):
    ...
    with open('expenses.csv', 'w', newline='') as file:
        ...
        for row in c.execute('SELECT category,'
                             ' amount FROM expenses_{}'.format(chat_id)).fetchall():
            writer.writerow(row)
    conn.close()
...
def handle_message(message):
    if message.text == '/start':
        create_table(message.chat.id)  # передаем chat_id в create_table()
        bot.reply_to(message, "Привет! Я бот для учета расходов.")
        bot.reply_to(message, "Напиши на что ты потратил деньги или выбери категорию из списка.",
        ...
        for row in c.execute('SELECT category, amount FROM expenses_{}'.format(message.chat.id)):
            stats += '{}: {}\n'.format(row[0], row[1])
        conn.close()
        ...
        if category in expenses:
            markup.add(category)
        msg = bot.reply_to(message, "Напиши сколько ты потратил на {}:".format(category),
                           reply_markup=markup)
        bot.register_next_step_handler(msg, process_amount, category)
    else:
        msg = bot.reply_to(message, "Некорректная категория. Выбери категорию из списка.",
                           reply_markup=get_categories_markup())
...
def process_amount(message, category):
    try:
        ...
        conn = sqlite3.connect('expenses.db')
        c = conn.cursor()
        c.execute('INSERT INTO expenses_{} (category, amount) VALUES (?, ?)'.format(message.chat.id),
                  (category,
                   amount))
        conn.commit()
        conn.close()
        ...
        msg = bot.reply_to(message, "ВЫ НЕ ВВЕЛИ СУММУ!.", reply_markup=get_categories_markup())
